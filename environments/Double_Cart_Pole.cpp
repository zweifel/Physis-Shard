...
void Double_Cart_Pole::double_cart_pole(double action, double *st, double *derivs)
{
...
void Double_Cart_Pole::rk4(double f, double y[], double dydx[], double yout[])
{
...
double Double_Cart_Pole::step(double* action)
{
  	double  dydx[6];
	...
	for(int i=0;i<2;++i)
	{
		...
		dydx[2] = state[3];
		dydx[4] = state[5];
		double_cart_pole(action[0],state,dydx);
		rk4(action[0],state,dydx,state);
	}
	...
	if(normalized_observation)
	{
		...
		observation[0]=state[0]/4.8;
		observation[1]=state[2]/0.52;
		observation[2]=state[4]/0.52;
	}
	else
	{
		//copy the state to the agents' observable variables
	...
	{
		observation[0]=state[0]/4.8;
		observation[1]=state[1]/2.0;
		observation[2]=state[2]/0.52;
		observation[3]=state[3]/2.0;
		observation[4]=state[4]/0.52;
		observation[5]=state[5]/2.0;
...
double Double_Cart_Pole::restart()
{
	trial++;
